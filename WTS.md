...
The format is **NUL-free JSON** (the local SQLite storage layer truncates strings at NUL bytes, so a NUL-delimited wire format cannot be persisted).

Rules:

| Rule | Description |
|------|-------------|
| **Wire format in `plaintext`** | For a reply message, `payload.messages[].plaintext` stores the full wire format above (prefix included), **not** the bare content. `content` is the reply text itself (JSON-escaped, so any chat text is safe). |
